...
class Quadrotor():
    def __init__(self):
        ...
        #To find speed of the motors
        self.motor_speed_sub=rospy.Subscriber("/crazyflie2/motor_speed",Actuators,self.callback)
    ...
    def smc_control(self, xyz, xyz_dot, rpy, rpy_dot):
        ...
        #print("xyz",[x-self.x_des,y-self.y_des,z-self.z_des])
        
        Fx=m*(-Kp*(x-self.x_des)-Kd*(xdot-self.x_desdot)+ self.x_desddot)
        Fy=m*(-Kp*(y-self.y_des)-Kd*(ydot-self.y_desdot)+ self.y_desddot)
        #print("The desiredforces are",[Fx,Fy])
        s1=(zdot-self.z_desdot)+lamda*(z-self.z_des)
        if s1>=0:
            sgns1=+1
        else:
            sgns1=-1

        u1=m*(g-lamda*(zdot-self.z_desdot)+ self.z_desddot -rhok*sgns1)*(1/(np.cos(r)*np.cos(p)))

        theta_des=asin(Fx/u1)
        phi_des=asin(-Fy/u1)
        print("Desired phi and theta",[phi_des,theta_des])
        print("u1",u1)

        s2=rd+lamda*np.arctan2(np.sin(r-phi_des),np.cos(r-phi_des))
        
        s3=pd+lamda*np.arctan2(np.sin(p-theta_des),np.cos(p-theta_des))
        s4=yawd+lamda*np.arctan2(np.sin(yaw),np.cos(yaw))
        ...
        u2=((-pd*yawd*((Iy-Ix)/Ix))+Ip*omega*pd/Ix-lamda*(rd)-rhok*sgns2)
        u3=((-rd*yawd*((Iz-Ix)/Iy))+Ip*omega*rd/Ix-lamda*(pd)-rhok*sgns3)
        u4=(((-rd*pd*((Ix-Iy)/Iz))-lamda*(yawd)-rhok*sgns4))
        
        U=np.array([u1,u2,u3,u4])

        print("The control inputs:",U)
        #U=np.transpose(U_matrix)
        #print([len(U),len(U[0])])
        vel_sq=self.alloc@U
        print("Velocity squared", vel_sq)
        ...
        #velocity=[vel_sq[0]**0.5, vel_sq[1]**0.5, vel_sq[2]**0.5, vel_sq[3]**0.5]
        #print(velocity)
        motor_vel=vel
        #print(vel)
        ...
        # # rospy.sleep(0.7)
        # # motor_speed.angular_velocities = [zero_vel[0], -1*zero_vel[1], zero_vel[2], -1*zero_vel[3]]
        # # self.motor_speed_pub.publish(motor_speed)
        
        # # rospy.sleep(0.5)
        # # print("zero velocity:",[self.w1,self.w2,self.w3,self.w4])
        self.rate.sleep()
...
#This function is used to find the angular velocities of the motors
    def callback(self,msg):
        self.w1=msg.angular_velocities[0]
        self.w2=msg.angular_velocities[1]
        self.w3=msg.angular_velocities[2]
        self.w4=msg.angular_velocities[3]
